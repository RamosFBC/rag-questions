...
user_prompt = st.text_area(
    "Escreva o que gostaria de cobrar na sua questão:", height=150, key="user_prompt"
)
...
if "question" in st.session_state and not st.session_state["edit_question"]:
    ...
    with st.form("edit_form"):
        ...
        updated_alternatives = st.text_area(
            "**Alternativas:**",
            value="\n".join(ast.literal_eval(str(st.session_state["alternatives"]))),
            height=200,
        )
        ...
        updated_learning_objective = st.text_area(
            "**Objetivo Educacional:**", value=st.session_state["learning_objective"]
        )
